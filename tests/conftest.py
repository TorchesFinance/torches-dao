...
def token(ERC20Impl, accounts):
    yield ERC20Impl.deploy("Torches DAO Token", "TOPTT", 18, 0, {"from": accounts[0]})
...
def mock_lp_token(MockCErc20, coin_deposit, accounts):  # Not using the actual Curve contract
    yield MockCErc20.deploy("Torches C deposit token", "cUSD", 18, coin_deposit, {"from": accounts[0]})

@pytest.fixture(scope="module")
def mock_lp_token_A(MockCErc20, coin_a, accounts):  # Not using the actual Curve contract
    yield MockCErc20.deploy("Torches C A token", "cUSDA", 18, coin_a, {"from": accounts[0]})

@pytest.fixture(scope="module")
def mock_lp_token_B(MockCErc20, coin_b, accounts):  # Not using the actual Curve contract
    yield MockCErc20.deploy("Torches C B token", "cUSDB", 18, coin_b, {"from": accounts[0]})
