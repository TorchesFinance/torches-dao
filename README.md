...
Vyper contracts used in the [Torches](https://www.torches.finance/) Governance DAO.
...
```bash
git clone https://github.com/TorchesFinance/torches-dao.git
cd torches-dao
...
3. Live deployment
    ...
    1. Create the new network folder under [./scripts](scripts) by copying one of the existing ones, the folder name is same as network id added to brownie.(example: brownie --network kcc-test, kcc-test is the network id)
    ...
    ```bash
    brownie run kcc-test/deploy deploy --network kcc-test
    ```
    ...
    This deploys and links all of the core Torches DAO contracts. **DO NOT MOVE OR DELETE THIS FILE**. It is required in later deployment stages.

### 3. Add new gauge

1. Torches DAO contracts must be deployed, and the deployment.json must exist
...
3. Run the deploy.py script:
    ...
    ```bash
    brownie run kcc-test/deploy add_gauge --network kcc-test
    ```
